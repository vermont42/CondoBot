...
- When sending JSON via `curl -d`, do NOT pass it inline — the shell mangles nested quotes. Write the JSON to a temp file and use `curl -d @/tmp/filename.json` instead. This is especially critical when the request requires HMAC signature verification: write the JSON to a file with the Write tool, compute the signature from that file (e.g. `tr -d '\n' < /tmp/payload.json | openssl dgst -sha256 -hmac "$SECRET"`), and send with `curl -d @/tmp/payload.json` so the signed bytes and sent bytes are identical.
...
- [ ] Compare outputs of Haiku, Sonnet, and Opus models for the same inquiries. The inquiry volume is low enough that, if there is a noticable quality difference in the more-expensive models, we should use them.
